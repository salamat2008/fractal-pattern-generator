...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.canvas = Widget(self.centralWidget())
    ...
    def start_(self):
        ...
        :return: None
        """
        self.canvas.n_iter = self.formLayout_fields['Глубина'].value()
    ...
    def sierpinski_triangle(self):
        ...
        self.formLayout_fields['Угол поворота'].setValue(60)
        self.formLayout_fields['Глубина'].setValue(3)
        self.formLayout_fields['Длинна перемещения'].setValue(self.formLayout_fields['Размер'].value())
    ...
    def open_file(self):
        ...
        if os.path.isfile(filename) and filename.endswith('.json'):
            try:
                ...
                fields = (self.formLayout_fields['Толщина'],
                          self.formLayout_fields['Глубина'],
                          self.formLayout_fields['Размер'],
                          self.formLayout_fields['Угол поворота'],
                          self.formLayout_fields['Длинна перемещения'],
                          self.formLayout_fields['Шаблон'])
                data = data[list(data.keys())[0]]
                if len(data) > 0:
                    for field, key in zip(fields, data.keys()):
                        if isinstance(field, QDoubleSpinBox | QSpinBox) and isinstance(data[key], int | float):
                            field.setValue(data[key])
                        elif isinstance(field, QLineEdit) and isinstance(data[key], str):
