...
class TestMTextlistwidget(unittest.TestCase):
    ...
    def testtakecurrentitem(self):
        for text in self.textlist:
            self.assertIsNone(self.widget.addtext(text))
        for text in self.textlist[len(self.textlist) // 2:][::-1]:
            self.assertEqual(self.widget.takecurrentitem().text(), text)
        self.assertListEqual(
            self.widget.gettextlist(), self.textlist[: len(self.textlist) // 2]
        )


class TestMColorlistwidget(unittest.TestCase):
    widget = MColorlistwidget()
    colorlist = [QColor(name) for name in QColor.colorNames()]

    def testaddcolor(self):
        for color in self.colorlist:
            self.assertIsNone(self.widget.addcolor(color))
        self.assertListEqual(self.widget.getcolorlist(), self.colorlist)
        self.assertIsNone(self.widget.clear())

    def testtakecurrentitem(self):
        for color in self.colorlist:
            self.assertIsNone(self.widget.addItem(color))
        for color in self.colorlist[len(self.colorlist) // 2:][::-1]:
            self.assertEqual(
                self.widget.takecurrentitem().text(), f"#{hex(color.rgb()).upper()[4:]}"
            )
        self.assertListEqual(
            self.widget.getcolorlist(), self.colorlist[: len(self.colorlist) // 2]
        )
